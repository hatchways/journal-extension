...
import { createJournalEntry } from "../store/thunks";
...
const AddEntry = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await dispatch(
      createJournalEntry({
        ...entryData,
        appliedOn: appliedOn.toString(),
        followUpDate: followUpDate?.toString() ?? undefined,
      })
    );
  };
        ...
        <Grid container>
          ...
          <Grid container spacing={3} className={classes.formRow}>
            ...
            <Grid item xs={6} sm={2} className={classes.dateColumn}>
              ...
              <DatePicker
                value={appliedOn}
                onChange={(value) => setAppliedOn(value ?? new Date())}
